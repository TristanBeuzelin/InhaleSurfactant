...
def f(x, params):
    ...
    return sigma * x / U0 - lamb * A / h1_lamb - 2 * (lamb ** 2) * B * h1_lamb / (1 - C * (1 - h1 - h1_lamb))
...
def f_prim(x, params):
    ...
    return sigma / U0 + lamb ** 2 * A * h1_lamb_prim / (h1_lamb ** 2) + 2 * lamb ** 2 * B * (lamb * h1_lamb_prim * (1 - C * (1 - h1)) - C * h1_lamb * h1_prim) / ((1 - C * (1 - h1 - h1_lamb) ** 2))
...
def main():
    ...
    for viscosity in viscosities:
        ...
        ax.plot(10 ** 6 * flow_rates, 10 ** 6 * volumes, label=f"{viscosity:.2f} Pa.s", color="black")
    lines = ax.get_lines()
    fontsize=15
    ...
    plt.ylabel(f"Volume (mL)", size=20)
    ...
    plt.savefig("/".join((SAVE_PATH, "viscocrit.svg")), dpi=500, format="svg", bbox_inches='tight')
    """-------------------------------"""
