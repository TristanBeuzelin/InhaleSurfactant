...
class Tree:
    ...
    def __init__(self, surfactant, a0, n_gen, track_ruptures=False, coating=True):
        """
        Parameters
        ----------
        ...
        if self.track_ruptures:
            ...
            self.split_ruptures = [
                np.ones(shape=(2**i,)) * n_gen for i in range(self.n_gen)
            ]
            self.coating_ruptures = [
                np.ones(shape=(2**i,)) * (-1) for i in range(self.n_gen + 1)
            ]
    ...
    def reset_(self):
        ...
        self.first_node_quat = Quaternion(0, *self.first_vec)
        qy = self.quat_exp(theta=-self.tree_phi / 2, vec=np.array((0, 1.0, 0)))
        qx = self.quat_exp(theta=self.tree_gamma / 2, vec=np.array((1.0, 0, 0)))
        uz = Quaternion(0, 0, 0, 1.0)
        normal_quat = (
            qy @ (qx @ uz @ qx.inv() / qx.squared_norm()) @ qy.inv() / qy.squared_norm()
        )
    ...
    def set_surfactant(self, mu, rho, sigma):
        """
    ...
    def quat_exp(self, theta, vec):
        ...
        return Quaternion(np.cos(theta), *sin_vec)
    ...
    def generate_next_level(self, level):
        ...
        for full_vec, vec, normal in zip(
            self.full_vecs[-1], self.vecs[-1], self.normals[-1]
        ):
            ...
            full_level.append(
                full_vec + left_vec * (6 * self.a0 * (self.lamb ** (level + 1)))
            )
            full_level.append(
                full_vec + right_vec * (6 * self.a0 * (self.lamb ** (level + 1)))
            )
    ...
    def inject(self, volume, flow_rate):
        """
    ...
    def coat(self, level):
        ...
        """
        self.H[level][np.logical_not(self.coated[level])] = 0.36 * (
            1
            - np.exp(
                -2
                * np.power(
                    (
                        self.surfactant.mu
                        * self.speeds[level][np.logical_not(self.coated[level])]
                        / self.surfactant.sigma
                    ),
                    0.523,
                )
            )
    ...
    def split(self, level):
        ...
        X = (2 * B0 * V1_tilde[new_mask] * (lamb**4)) / (
            Re[new_mask] * Ca[new_mask] * (lamb**2)
            + 16 * V1_tilde[new_mask] * Ca[new_mask]
        )
        self.alphas[level][new_mask] = (
            1
            - (X * np.sin(self.theta) * np.sin(self.phis[level][new_mask]))
            / (1 - X * np.cos(self.theta) * np.sin(self.gammas[level][new_mask]))
        ) / 2

        self.alphas[level][self.alphas[level] < 0.0] = 0.0
        self.alphas[level][self.alphas[level] > 1.0] = 1.0

        self.speeds[level + 1][self.uphills[level]] = (
            self.speeds[level] * self.alphas[level] / (lamb**2)
        )
        self.speeds[level + 1][np.logical_not(self.uphills[level])] = (
            self.speeds[level] * (1 - self.alphas[level]) / (lamb**2)
        )

        self.volumes[level + 1][self.uphills[level]] = self.alphas[level] * (
            self.volumes[level] - Rd * Va
        )
        self.volumes[level + 1][np.logical_not(self.uphills[level])] = (
            1 - self.alphas[level]
        ) * (self.volumes[level] - Rd * Va)
        ...
        if self.track_ruptures:
            if level == 0:
                ...
                idx_3 = (self.volumes[level + 1][self.uphills[level]] == 0.0) & (
                    V1_tilde != 0.0
                )
                ...
                idx_3 = np.full(2**level, False)
                idx_3 = (self.volumes[level + 1][self.uphills[level]] == 0.0) & (
                    (self.volumes[level] - Rd * Va) > 0.0
                )
                self.split_ruptures[level][idx_3] = level

                # Coating ruptures
                idx_4 = np.full(2**level, False)
                idx_4 = (self.volumes[level] != 0.0) & (
                    (self.volumes[level] - Rd * Va) <= 0.0
                )
    ...
    def final_coat(self):
        ...
        Va = np.pi * np.power(a, 2) * 6 * a
        self.H[-1][np.logical_not(self.coated[-1])] = 0.36 * (
            1
            - np.exp(
                -2
                * np.power(
                    (
                        self.surfactant.mu
                        * self.speeds[self.n_gen][np.logical_not(self.coated[-1])]
                        / self.surfactant.sigma
                    ),
                    0.523,
                )
    ...
    def compute_std_inv(self):
        ...
        if self.efficiency == 0.0:
            ...
            self.homogeneity = (
                100
                * self.final_volumes
                / (self.efficiency * self.injected_volume / (2**self.n_gen))
            )
