...
class Tree_plot:
    def __init__(self, n_gen, first_vec, phi, gamma):
        self.n_gen = n_gen
        self.first_vec = first_vec
        self.tree_phi = phi
        self.tree_gamma = gamma

        self.first_quat = Quaternion(0, *self.first_vec)
    ...
    def quat_exp(self, theta, vec):
        ...
        quat = Quaternion(np.cos(theta), *sin_vec)
        return quat
    ...
    def generate_next_level(self, a, fig, ax):
        ...
        new_level_vecs = []
        new_level_normals = []
        for vec, normed_vec, normal_vec in zip(
            self.vecs[-1], self.norm_vecs[-1], self.normals[-1]
        ):
            ...
            right_normal_vec = np.array(
                (right_normal.i, right_normal.j, right_normal.k)
            )
    ...
    def plot_end(self, volumes, path, filename):
        x = []
        y = []
        z = []
        vols = []
        ...
        scatter = self.ax.scatter(
            x, y, z, linewidth=1, c=vols, cmap=cm.plasma, alpha=0.5
        )
    ...
    def animate(self, path, filename):
        ...
        def animate(i):
            self.ax.view_init(elev=0.0, azim=2 * i)
            return ()
